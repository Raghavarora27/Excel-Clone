...
$(document).ready(function () {
  // Iske andar ka code tab chalega jab puri site load ho jayegi
  let cellContainer = $(".input-cell-container");

  // Column and row bana rhe h
  for (let i = 1; i <= 100; i++) {
    let ans = "";
  ...
  // Making all the cells of input cell container
  // har baar ek new row banake , usme sare column append karke, use row ko input cell container me append kar rhe h
  for (let i = 1; i <= 100; i++) {
    let row = $(`<div class="cell-row"></div>`);
    for (let j = 1; j <= 100; j++) {
      console.log($(`.colId-${j}`).attr("id"));
      let colCode = $(`.colId-${j}`).attr("id").split("-")[1]; // we will get A B C D and so on......
      let column = $(
  ...
  $(".align-icon").click(function () {
    ...
    $(this).addClass("selected"); // apne upar selected class laga rhe h
  });

  $(".style-icon").click(function () {
    // selected class h toh remove ho jayegi , varna lag jayegi
    $(this).toggleClass("selected");
  });

  $(".input-cell").click(function (e) {
    // cell ko select karne ke liye
    if (e.ctrlKey) {
      ...
      if (rowId < 100) {
        // max value of row and column is 100 in my code
  ...
  // Removing the focus from one cell ,when we click on the another cell
  $(".input-cell").blur(function () {
    $(".input-cell.selected").attr("contenteditable", "false");
    updateCell("text", $(this).text());
  });

  // Scroll Bar bottom and Left
...
// finding the row id and column id of the passed cell
...
// Updating the cell
// By adding the updated properties to the cell
  ...
  $(".input-cell.selected").each(function () {
    ...
    if (cellData[selectedSheet][rowId]) {
      ...
      if (cellData[selectedSheet][rowId][colId]) {
        ...
        // if column doesnot exists
        // making the new column and then adding the properties
        cellData[selectedSheet][rowId][colId] = { ...defaultProperties };
        cellData[selectedSheet][rowId][colId][property] = value;
      }
    } else {
      // if row doesnot exists
      // making the new row and column and then adding the properties
      cellData[selectedSheet][rowId] = {};
      cellData[selectedSheet][rowId][colId] = { ...defaultProperties };
      cellData[selectedSheet][rowId][colId][property] = value;
    }
